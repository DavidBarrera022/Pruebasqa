from .intermediate_functions import fill_na,definir_cols_types,agrupacion_target,cols_minuscula,read_yaml_storage,read_bq,df_to_storage_parquet
from .feature_functions import calcular_edad,calcular_generacion,calcular_grupo_etario,read_parquet,feature_to_storage_parquet
import argparse
from .globals import fc

def handle_error_and_create_bucket_mf(error_msg, e=None):
...
def intermediate_function(df, params):
    try:
        ...
        handle_error_and_create_bucket_mf("Error in intermediate_function", e)
...
def main_intermediate(blob_path, blob_path_inter, output_path_intermediate):
    try:
        ...
        if "blob_path" in str(e) or "blob_path_inter" in str(e):
            handle_error_and_create_bucket_mf(".yml file not found at path", e)
    except Exception as e:
        handle_error_and_create_bucket_mf("Unexpected error in main_intermediate", e)
...
def main_feature_processing(blob_path, path_intermediate, nombre_archivo_parquet, output_path):
    try:
        ...
        if df_intermediate.empty:
            handle_error_and_create_bucket_mf("No data found in the intermediate parquet file.")
        ...
        handle_error_and_create_bucket_mf("Unexpected error in main_feature_processing", e)
...
def main(blob_path, blob_path_inter, output_path_intermediate, output_path):
    try:
        ...
        handle_error_and_create_bucket_mf("Unexpected error in the main process", e)
